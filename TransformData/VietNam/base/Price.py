import pandas as pd
import sys
sys.path.append(r'D:\DataVietNam - Copy - Copy')
...
class TransformPrice(Price):
    ...
    def getDataCafeF(self,symbol):
        ...
        try:
            data = data[["Ngay","GiaDongCua"]]
        except:
            data = pd.DataFrame({"Ngay":[],"GiaDongCua":[]})
        return data[["Ngay","GiaDongCua"]].rename(columns={"Ngay":"Date","GiaDongCua":"Close"})
