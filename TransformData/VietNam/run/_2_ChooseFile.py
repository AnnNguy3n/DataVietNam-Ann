    ...
    for i in list_symbol[::50]:
        for index in range(len(F_RANGE)-1,-1,-1):
            ...
            if read_file(path,file_type,"BalanceSheet",source) == True:
                ...
                break
        temp_stack += 50
        progress_bar(temp_stack,TOTAL,text=f"Lấy {type_data} --- {source}---{type_time}")
        ...
        for symbol in list_symbol:
            ...
            for part in FU.DividendPartObject:
                if part == "CashDividend":
                    ...
                    # print("Khong dinh truong hop nao !!!!!!!!!!!!!!!!")
                    df = pd.DataFrame()
...
if __name__ == "__main__":
    ...
    FileVersionDividendNew(SYMBOL,F_RANGE,"VietStock")

    '''Lấy data Finance Quý từ ingestion'''
